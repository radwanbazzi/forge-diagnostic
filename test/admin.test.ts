...
describe('admin stubs remain 501 behind auth', () => {
	it('PATCH /leads/:id → 501 (authenticated)', async () => {
		const res = await SELF.fetch(`https://x/api/admin/leads/${crypto.randomUUID()}`, { method: 'PATCH', headers: auth });
